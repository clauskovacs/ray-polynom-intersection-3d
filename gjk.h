// gjk.h -> last modified: 2016_04_07


/*
<<--  gjk(shape *A, shape *B, int dim_a, int dim_b);  -->>

determine the collision of two convex shapes in 3d
...
/*
<<--  ray_polygon_intersection(shape *p, int dim_p, shape ray_origin, shape point_dir)  -->>

shape p[] = {p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, p3.x, p3.y, p3.z, p4.x, p4.y, p4.z, p5.x, p5.y, p5.z, p6.x, p6.y, p6.z};

int dim_p = sizeof(p)/sizeof(*p);

r_origin -> origin of the ray
r_point -> point where the ray is pointing to

shape collision = ray_polygon_intersection(p, dim_p, r_origin, r_point); // -> return value: x/y/z = -INFINITY -> no collision
*/
...
class shape
{
	public:
		...
		shape ray_polygon_intersection(shape *p, int dim_p, shape ray_origin, shape point_dir); // P ... points of the polygon, dim_p ... amount of points(x/y/z) 
...
bool gjk(shape *A, shape *B, int dim_a, int dim_b); // main - GJK function (2 polygons -> gjk intersection)

shape ray_polygon_intersection(shape *p, int dim_p, shape ray_origin, shape point_dir); // ray-polygon-intersection
