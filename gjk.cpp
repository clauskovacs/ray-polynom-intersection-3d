...
// determines, whether the ray hits a polynom (is so -> the point of intersection is returned)
// the polygon must be "flat" (all points must be elements of an area)
shape ray_polygon_intersection(shape *p, int dim_p, shape ray_origin, shape point_dir) // P ... points of the polynom, dim_p ... amount of points(x/y/z) defining the polynom, ray_origin ... x/y/z-coordinates where the ray starts, point_dir ... direction where the ray "points"
{
	shape return_point;
	return_point.x = -INFINITY;
	return_point.y = -INFINITY;
	return_point.z = -INFINITY;
	...
	shape ray_dir;
	ray_dir.x = point_dir.x - ray_origin.x;
	ray_dir.y = point_dir.y - ray_origin.y;
	ray_dir.z = point_dir.z - ray_origin.z;
	// calculate ray direction


	// n -> Surface_normal of the polygon
	shape p1p2; // vector from point p[0] to p[1]
	p1p2.x = p[1].x - p[0].x;
	p1p2.y = p[1].y - p[0].y;
	p1p2.z = p[1].z - p[0].z;
	...
	shape n = p1p2.cross(p1p3);
	// n -> Surface_normal of the polygon

	// line surface intersection
	float parallel_check = ray_dir.dot(n);

	if (parallel_check == 0)
	{
//		std::cout << "ray and area(polygon) are parallel" << std::endl;
	}

	shape p0mlo; // p_0 - l_0
	p0mlo.x = p[0].x - ray_origin.x;
	p0mlo.y = p[0].y - ray_origin.y;
	p0mlo.z = p[0].z - ray_origin.z;

	float enumerator = p0mlo.dot(n);

	if (enumerator == 0)
	{
//		std::cout << "ray and area(polygon) are identical (ray lies in the area)" << std::endl;
	}
	...
	if (enumerator != 0 and parallel_check != 0) // line and polygon are (a) not identical and (b) not parallel
	{
		float d = enumerator / parallel_check;

		c_p.x = d * ray_dir.x + ray_origin.x;
		c_p.y = d * ray_dir.y + ray_origin.y;
		c_p.z = d * ray_dir.z + ray_origin.z;
		...
		// check mass middle point to collision_point vs all sides of the polygon
		shape s1, s2;
		int amount_collision = 0; // amount of intersections -> even ... ouside ; odd ... inside

		for (int m = -1 ; m < dim_p-1 ; m++)
		{
			if (m == -1)
			{
				s1.x = p[0].x;
			...
			z = - s1.y * (mmp.x - c_p.x) + c_p.y * (mmp.x - c_p.x) + s1.x * (mmp.y - c_p.y) - c_p.x * (mmp.y - c_p.y);
			t = (s2.y - s1.y) * (mmp.x - c_p.x) - (s2.x - s1.x) * (mmp.y - c_p.y);

			float div = z/t;

			pin.x = s1.x + div * (s2.x - s1.x);
			pin.y = s1.y + div * (s2.y - s1.y);
			pin.z = s1.z + div * (s2.z - s1.z);

			float s = (s1.x - c_p.x + div * (s2.x - s1.x) ) / (mmp.x - c_p.x);

			if (s > 0 and div > 0 and div < 1)
			{
				amount_collision++;
			}
		}

		if(amount_collision&1) // odd
		{
			return_point.x = c_p.x;
			return_point.y = c_p.y;
			return_point.z = c_p.z;
		}
		else // even
		{
		}
